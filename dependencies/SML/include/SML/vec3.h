...
namespace sml
{
    ...
    class alignas(simdalign<T>::value) vec3
    {
        public:
            ...
            // Operations
            SML_NO_DISCARD inline constexpr T dot(const vec3& other) const noexcept
            {
                if constexpr (std::is_same<T, f32>::value)
                {
                    __m128 me = _mm_load_ps(v);
                ...
                /*if constexpr (std::is_same<T, f64>::value)
                {
                    ...
                    __m256d dp = _mm256_dp_ps(me, ot, 0x7f);

                    return _mm256_cvtsd_f64(dp);
                }*/
            ...
            SML_NO_DISCARD inline constexpr T sqrt() const noexcept
            {
                ...
                if constexpr (std::is_same<T, f64>::value)
                {
                    __m256d t = _mm256_load_pd(v);
                    __m256d res = _mm256_sqrt_pd(t);
